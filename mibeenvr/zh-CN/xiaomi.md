...
> `xiaomi:` 段没有开关字段 —— 只要配置了凭据即启用。局域网设备自动发现由顶层 [`auto_discover`](https://github.com/Mi-Bee-Studio/MiBeeNvr/blob/v0.13.0/docs/zh/configuration.md) 控制，与小米账户发现（Web UI 扫描面板）是两回事。
...
> `frame_watchdog_timeout` 与 `health_overrides`（见 [configuration.md](https://github.com/Mi-Bee-Studio/MiBeeNvr/blob/v0.13.0/docs/zh/configuration.md)）接管。
