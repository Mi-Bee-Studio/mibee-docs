...
- **[子码流按需](https://github.com/Mi-Bee-Studio/MiBeeNvr/blob/main/docs/zh/sub-stream.md)**：宫格「流畅」模式走低分辨率子码流，无观看者自动停拉零成本；ONVIF 子流自动发现，取不到自动回退主码流
...
- **[自适应录制](https://github.com/Mi-Bee-Studio/MiBeeNvr/blob/main/docs/zh/adaptive-recording.md)**：画面安静时自动降为延时摄影级稀疏写入，检测到活动 / 异常声音 / 外部触发立即恢复全帧率——静态场景磁盘占用直降 75%–98%，事件第一帧不丢
...
- **[按相机存储与热迁移](https://github.com/Mi-Bee-Studio/MiBeeNvr/blob/main/docs/zh/storage-management.md)**：运行时切换存储根 / 为单路相机指定专属盘，历史录像后台限速迁移（可配时间窗），全程无需重启；数据库与录像根解耦，换盘不丢索引
