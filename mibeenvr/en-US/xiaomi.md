...
> The `xiaomi:` section has no on/off switch — integration is active whenever credentials are configured. LAN auto-discovery is controlled by the top-level [`auto_discover`](config.md) setting and is separate from Xiaomi-account discovery (the Web UI scan panel).
...
> `frame_watchdog_timeout` and `health_overrides` (see [configuration.md](config.md)).
