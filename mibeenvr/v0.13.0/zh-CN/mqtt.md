...
不受「启用录制」开关限制的限时录制窗口——窗口期内**写盘录像**，到期自动收尾当前段并回到仅直播模式。适合「平时仅直播、事件时录一段」的场景（如 Home Assistant 移动触发）：
...
- `duration` 支持 Go 时长格式（`"30s"`、`"5m"`）；无效或非正值按缺省处理（回退为普通 `record`，并打 WARN 日志）
- 相机未运行时先按 `record` 语义启动取流，再开启窗口；运行中的相机原地续窗（重复触发取更晚的到期时间，与音频/像素触发的 hold 语义一致）
- 支持 H.264/H.265、MJPEG、HTTP-JPEG 相机；推送接入型（SRT/RTMP ingest 等）不支持，会返回明确错误
