...
The repository ships a ready-to-use Docker Compose config template at [`configs/config.docker.yaml`](https://github.com/Mi-Bee-Studio/MiBeeSteward/blob/main/configs/config.docker.yaml). It pre-configures paths and network settings appropriate for containerized deployment — copy it as your starting `config.yaml` and adjust `auth.jwt_secret` and `auth.initial_admin_password` for production.
