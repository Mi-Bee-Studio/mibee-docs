...
仓库提供了现成的 Docker Compose 配置模板 [`configs/config.docker.yaml`](https://github.com/Mi-Bee-Studio/MiBeeSteward/blob/main/configs/config.docker.yaml)，预配置了适合容器化部署的路径和网络设置——将其复制为你的起始 `config.yaml`，并为生产环境调整 `auth.jwt_secret` 和 `auth.initial_admin_password`。
