...
门面访问器不变）；迁移映射见 [v2-architecture.md](onvif-go-v2-architecture.md)。
...
共享，门面层无需加锁（见 [并发模型](onvif-go-concurrency.md)）。
...
单一可审计路径也让并发契约可证明——梯队语义详见[鉴权与安全](onvif-go-authentication.md)。
