...
这里是**从产品中抽离出来的开源库**的统一文档：GB28181 与 ONVIF 协议库（Go / Rust / C 实现）和指纹规则引擎。它们驱动着 MiBeeNvr、rpi-cam、ESP32 相机固件等产品，也可独立用于你自己的项目。
...
flowchart TB
    subgraph products [产品侧]
        ...
        ESPCAM[MiBee Cam 固件<br/>ESP32 相机]
    ...
    subgraph libs [本集合的库]
        ...
        OVCC[onvif-c<br/>ONVIF 设备服务端<br/>ESP-IDF / 纯 C]
...
| [onvif-c](onvif-c-quickstart.md) | C | 面向 ESP-IDF 的 ONVIF 设备服务端：SOAP Device/Media、Pull-Point MotionAlarm 事件、WS-Discovery —— 约 10 KB、零第三方依赖 | MiBee Cam 固件 |
...
- **ONVIF 互通**：需要客户端（发现并管理相机）用 `onvif-go`；需要把设备做成 ONVIF 相机用 `onvif-rs`（Linux）或 `onvif-c`（ESP-IDF 固件）
