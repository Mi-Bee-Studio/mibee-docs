#!/usr/bin/env python3
# =============================================================================
# mibee-docs 合并前泄密扫描（治理规范 §2.4a 强制门禁）
#
# 用法：
#   python3 scripts/security-check.py            # 扫描全仓库（CI 与本地自查）
#   python3 scripts/security-check.py --files a.md b.md   # 只扫指定文件
#
# 规则（命中任意一条即失败，退出码 1）：
#   A. 真实内网网段（192.168.62/63.x，本项目公开脱敏规范认定的真实家庭网段）
#   B. 已知基础设施地址（生产服务器等）
...
#   F. 凭据赋值白名单：示例中的 password/密码/secret/token 取值只允许
#      ① 三个官方公开默认密码 ② 厂商默认值 ③ 明确占位符。
#      新增取值必须在 ALLOWED_PASSWORDS 登记并在 PR 说明理由，由守门人审查。
#
# 误报处理：确属合理的值 → 加入对应白名单（提交在同一个 PR 里，守门人可见）。
# 白名单外的任何疑似泄密由官网团队（守门人）一票否决（治理规范 §3.1 红线）。
# =============================================================================
...
FORBIDDEN_SUBNETS = [
    ipaddress.ip_network("192.168.62.0/24"),
    ipaddress.ip_network("192.168.63.0/24"),
]

# ---- B. 已知基础设施地址（生产/内网服务，禁止出现在公开文档） --------------
FORBIDDEN_HOSTS = {
    "8.134.136.138",   # 官网生产服务器（阿里云）
}

# ---- C. 已知泄露标识（真实 SSID / 网名 / 设备名，按审计持续补充） ----------
FORBIDDEN_STRINGS = [
    "MickeyMiRoute",    # 真实家庭 WiFi SSID（2026-09-05 审计）
]
...
OFFICIAL_DEFAULT_PASSWORDS = {
    "mibeecam2026",      # MiBeeCam 全家族（AP/Web/GB28181）
    "mibeestudio2026",   # MiBee Studio 服务端产品（MiBeeHive 等）
    "mibeehome2026",     # MBHData 家庭平台（Web/MQTT/DB）
}
...
SCAN_EXTS = {".md", ".json", ".yaml", ".yml", ".toml", ".sh", ".py"}
SCAN_SKIP_DIRS = {".git", "node_modules", "scripts"}
...
def main() -> int:
    ap = argparse.ArgumentParser(description="mibee-docs 合并前泄密扫描")
