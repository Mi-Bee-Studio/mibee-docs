...
All JSON endpoints share one envelope: success `{"ok":true,"data":...}` + HTTP 200; failure `{"ok":false,"error":"<msg>"}` + 400/401/404/500/503. Writes authenticate with the `X-Password` header. The family-wide default password is `mibeecam2026` (a public initial password — change it right after first setup; empty and <6-char passwords are rejected). Password changes go through `POST /api/config` with `{"web_password":"..."}` (the old password is verified implicitly via `X-Password`). Password fields are masked `"****"` in GET responses; posting the mask back means "unchanged". CORS is fully open (`OPTIONS /*` → 204).
