...
| RTSP 服务器 | `rtsp_server.cpp` | RTSP 服务器（MJPEG+G.711 双轨道，无认证，端口 554） | `rtsp_server_init()`, `rtsp_server_start()` |
...
MJPEG 流服务运行在**端口 81** 作为独立 TCP 服务器，与端口 80 的主 httpd 完全分离。详见[端口分离设计](seeed-port-separation-design.md)。
...
| POST | `/api/files/batch` | | 批量删除文件 |
| DELETE | `/api/files` | | 删除指定文件 |
| GET | `/api/download?name=xxx` | 否 | 下载录像文件 |
| GET | `/api/scan` | 否 | 扫描 WiFi AP |
| POST | `/api/time` | | 手动设置时间 |
| POST | `/api/record?action=start\|stop` | | 录像控制 |
| POST | `/api/ota` | | 通过 URL 触发 OTA 固件更新 |
| POST | `/api/format` | | 格式化 SD 卡（需确认） |
...
认证：无——契约 v1.9（2026-09-18）已移除设备级 Web 密码，端点在可信局域网内开放（边界为路由器的 WPA2）。
